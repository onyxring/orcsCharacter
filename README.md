...
You might think you could use `cacheAsync()` and `cacheRepeatingAsync()` to populate your own object wrapper, and you'd be right.  But `cacheMultipleAsync() `is more than just syntactic sugar.  The above example bundles the retrieval of our specified basic attributes and both repeating sections into a single call.  Additionally, once changes have been made, the following will save all changes in a single call, regardless of which tracked objects they were made to:
